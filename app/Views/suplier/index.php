...
<?= $this->section('content') ?>
    <div class="row g-3">
        <div class="col-12 col-xl-4">
            <div class="panel">
                <div class="panel-title"><?= empty($editRow) ? 'Tambah Suplier' : 'Edit Suplier' ?></div>
                <p class="panel-muted mb-3">Kelola data suplier untuk kebutuhan stok masuk.</p>

                <?php if (! $hasTable): ?>
                    <div class="alert alert-danger mb-0">Tabel tb_suppliers belum tersedia pada database.</div>
                <?php elseif (! $isAdmin): ?>
                    <div class="alert alert-warning mb-0">Mode Read Only untuk kasir.</div>
                <?php else: ?>
                    <form method="post" action="<?= empty($editRow) ? base_url('/suplier/store') : base_url('/suplier/update/' . (int) $editRow['id']) ?>" class="row g-2">
                        <?= csrf_field() ?>
                        ...
                        <div class="col-12 d-flex gap-2 mt-2">
                            ...
                            <?php if (! empty($editRow)): ?>
                                <a href="<?= base_url('/suplier') ?>" class="btn btn-outline-light">Batal Edit</a>
                            <?php endif; ?>
                        </div>
                    </form>
                <?php endif; ?>
        ...
        <div class="col-12 col-xl-8">
            <div class="panel">
                <div class="d-flex flex-column flex-lg-row justify-content-between gap-2 mb-3">
                    <div>
                        <div class="panel-title mb-1">Daftar Suplier</div>
                        <p class="panel-muted mb-0">Referensi utama saat input stok masuk.</p>
                    </div>

                    <?php if ($hasTable): ?>
                        <form method="get" action="<?= base_url('/suplier') ?>" class="d-flex gap-2">
                            <input type="text" name="search" value="<?= esc((string) $search) ?>" class="form-control" placeholder="Cari nama suplier...">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-dark table-hover align-middle" style="--bs-table-bg: transparent; --bs-table-hover-bg: rgba(255,255,255,0.06);">
                        ...
                        <tbody>
                            <?php if (! $hasTable): ?>
                                <tr>
                                    <td colspan="<?= $columnCount ?>" class="text-center text-danger py-4">Tabel tb_suppliers belum tersedia.</td>
                                </tr>
                            <?php elseif (empty($rows)): ?>
                                <tr>
                                    <td colspan="<?= $columnCount ?>" class="text-center text-secondary py-4">Belum ada data suplier.</td>
                                </tr>
                            <?php else: ?>
                                        ...
                                        <td>
                                            <?php if ($isAdmin): ?>
                                                <div class="d-flex gap-1">
                                                    <a href="<?= base_url('/suplier?edit=' . (int) $row['id']) ?>" class="btn btn-sm btn-outline-info">Edit</a>
                                                    <form method="post" action="<?= base_url('/suplier/delete/' . (int) $row['id']) ?>" onsubmit="return confirm('Hapus suplier ini?');">
                                                        <?= csrf_field() ?>
